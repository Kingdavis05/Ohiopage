// server.js — ONE FILE WEB SERVICE with LKQ + optional CarParts feed + Body Parts category
// Start: node server.js  (Render will pass process.env.PORT)
// -------------------- Configure (Render -> Environment) --------------------
// LKQ (authorized feed):
//   LKQ_API_URL      = https://<lkq-endpoint>/v1/parts?in_stock=true
//   LKQ_API_TOKEN    = <bearer token> (OR use LKQ_USERNAME + LKQ_PASSWORD for Basic Auth)
//   LKQ_USERNAME     = <optional>
//   LKQ_PASSWORD     = <optional>
//   LKQ_API_KEY      = <optional x-api-key>
// CarParts (authorized feed you control; NOT scraping carparts.com):
//   CARPARTS_API_URL   = https://<your-authorized-feed>/parts?category=body
//   CARPARTS_API_TOKEN = <optional bearer>
//   CARPARTS_USERNAME  = <optional basic auth user>
//   CARPARTS_PASSWORD  = <optional basic auth pass>
//   CARPARTS_API_KEY   = <optional x-api-key>
//
// NOTE: We do NOT scrape carparts.com. If you have permissioned/partner feeds, point CARPARTS_API_URL to those.
// If feeds are missing, we fall back to db.json, then to a built-in demo catalog with body parts + images.
...
function authHeaders(prefix) {
  ...
  const APIK = process.env[`${prefix}_API_KEY`];
  if (TOKEN) h["Authorization"] = "Bearer " + TOKEN;
  if (USER && PASS) h["Authorization"] = "Basic " + Buffer.from(USER + ":" + PASS).toString("base64");
  if (APIK) h["x-api-key"] = APIK;
  return h;
}

/* ------------------- Normalizers ------------------- */
// Normalize generic -> { id, name, image, base_price, year, make, model, category }
function norm(core, extra = {}) {
  const veh = core.vehicle || {};
  const id =
    core.id ?? core.partId ?? core.partID ?? core.sku ?? core.partNumber ?? ("item-" + Math.random().toString(36).slice(2));
  ...
  // Try to infer category (body vs mechanical) if not present
  let category =
    core.category ?? extra.category ??
...
async function readProductsLKQ() {
  const lkqUrl = process.env.LKQ_API_URL;
  if (!lkqUrl) return [];
  const data = await httpGetJson(lkqUrl, authHeaders("LKQ"));
  return normalizeArrayLike(data); // mixed categories
}

async function readProductsCarParts() {
  const cpUrl = process.env.CARPARTS_API_URL;
  if (!cpUrl) return [];
  const data = await httpGetJson(cpUrl, authHeaders("CARPARTS"));
  // force category=body if your feed isn't labeled
  ...
  try {
    ...
    if (fs.existsSync(p)) {
      ...
      if (Array.isArray(json?.products) && json.products.length) {
        return normalizeArrayLike(json.products);
      }
    }
  } catch (e) {
...
function defaultCatalog() {
  // Includes mechanical + body parts with safe placeholder images (royalty-free placeholders).
  return [
    // Mechanical
    ...
    { id: "left-fender",      name: "Fender (Driver Side)",    image: "https://picsum.photos/seed/fender/800/480",        base_price: 129.00, year: 2019, make: "Honda",  model: "Civic", category: "body" },
    ...
    { id: "side-mirror",      name: "Side Mirror (RH)",        image: "https://picsum.photos/seed/mirror/800/480",        base_price: 89.00,  year: 2017, make: "Chevy",  model: "Malibu",category: "body" },
    { id: "tail-light",       name: "Tail Light Assembly",     image: "https://picsum.photos/seed/taillight/800/480",     base_price: 119.00, year: 2015, make: "BMW",    model: "328i",  category: "body" },
  ...
  for (const list of lists) {
    for (const p of list) {
      ...
      if (!seen.has(id)) {
        ...
        // merge shallow, prefer existing image if new one missing
...
async function loadProducts(){
  try{
    ...
    PRODUCTS = list.map((p)=>({
      ...
      make: p.make ?? "",
      model: p.model ?? "",
      category: p.category ?? (/bumper|fender|hood|grille|mirror|tail|headlight|door|panel/i.test((p.name||"")) ? "body" : "mechanical")
    ...
    PRODUCTS = [
      ...
      {id:"left-fender",name:"Fender (Driver Side)",image:"https://picsum.photos/seed/fender/600/360",year:2019,make:"Honda",model:"Civic",category:"body",price:179.00}
    ];
  }
...
if(payBtn){ payBtn.addEventListener('click', ()=>{
  alert('✅ Order placed (demo). Add Stripe/PayPal on the server later.');
...
const server = http.createServer(async (req, res) => {
  ...
  if (pathname === "/api/products" && req.method === "GET") {
    try {
      const [lkq, cp, db] = await Promise.allSettled([
        readProductsLKQ(),
        readProductsCarParts(),
        Promise.resolve(readProductsDB()),
      ]);

      const lkqItems = lkq.status === "fulfilled" ? lkq.value : [];
      const cpItems  = cp.status  === "fulfilled" ? cp.value  : [];
      const dbItems  = db.status  === "fulfilled" ? db.value  : readProductsDB();

      const merged = dedupeMerge(
        // Prefer body parts from CarParts feed if present
        cpItems,
        lkqItems,
        dbItems.length ? dbItems : defaultCatalog()
      );

      return send(res, 200, "application/json; charset=utf-8", JSON.stringify(merged));
    } catch (e) {
  ...
  if (pathname === "/api/orders" && req.method === "POST") {
    ...
    req.on("end", () => {
      // TODO: validate + store + charge (Stripe/PayPal)
